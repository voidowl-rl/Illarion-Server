...
class LuaPlayerChatScript : public LuaScript {
public:
    LuaPlayerChatScript();
    explicit LuaPlayerChatScript(const std::string &filename);

    virtual ~LuaPlayerChatScript();
...
private:
    LuaPlayerChatScript(const LuaPlayerChatScript &);
    LuaPlayerChatScript &operator=(const LuaPlayerChatScript &);
};
